...
function getOldStockCode(){
	var oldData = [];
	db.enterpriseData.find().toArray(function(err,result){
	...
	for(var k in jsonData){
		...
		for(var i=0,ilen=stock.length;i<ilen;i++){
			...
			var insertData = {
				...
				stockName:stock[i].enterpriseName,
			}
...
ep.all("getOldStockCode","filterJsonData",function(){
	...
	db.enterpriseData.insert(data,function(err,result){
		if(err){
			console.log({
				code : "err",
				err : err,
